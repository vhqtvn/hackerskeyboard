...
import vn.vhn.pckeyboard.LatinKeyboardBaseView.OnKeyboardActionListener;
...
public class PointerTracker {
    ...
    private static List<Keyboard.Key> sSlideKeys = new ArrayList<Keyboard.Key>(10);
    ...
    void sendSlideKeys() {
        ...
        if ((slideMode & 4) > 0) {
            ...
            if (n > 0 && (slideMode & 1) > 0) {
                Keyboard.Key key = sSlideKeys.get(0);
                detectAndSendKey(key, key.x, key.y, -1);
            }
            if (n > 1 && (slideMode & 2) > 0) {
                Keyboard.Key key = sSlideKeys.get(n - 1);
                detectAndSendKey(key, key.x, key.y, -1);
            }
        }
        clearSlideKeys();
    ...
    public void onMoveEvent(int x, int y, long eventTime) {
        ...
        final Keyboard.Key oldKey = getKey(keyState.getKeyIndex());
        if (isValidKeyIndex(keyIndex)) {
    ...
    private void detectAndSendKey(Keyboard.Key key, int x, int y, long eventTime) {
